...
export default function CustomizePage() {
  ...
  const toggleFruit = (f: string) => {
    if (fruits.includes(f)) { setFruits(prev => prev.filter(x => x !== f)); return; }
    if (fruits.length >= maxFruits) { setErr(`Max ${maxFruits} fruit${maxFruits > 1 ? 's' : ''} allowed`); return; }
  ...
  return (
    <main className="space-y-5 pb-8">
      ...
      {/* FRUIT */}
      <section className="rounded-2xl border border-slate-700 bg-slate-900/40 p-4 space-y-3">
        <h2 className="font-semibold text-white">Fruit <span className="text-slate-500 text-xs">optional · max {maxFruits}</span></h2>
        <div className="flex gap-2 flex-wrap">
          {FRUITS.map(f => (
            <button key={f} type="button" onClick={() => toggleFruit(f)}
              className={`px-3 py-2 rounded-xl border text-sm transition ${fruits.includes(f) ? 'bg-emerald-700 border-emerald-500 text-white' : 'bg-slate-800 border-slate-600 text-slate-300 hover:border-slate-400'}`}>
              {f}
            </button>
          ))}
        </div>
        {fruits.length > 0 && <p className="text-emerald-400 text-xs">{fruits.join(', ')} selected</p>}
      </section>
